...
export class StaffComponent implements OnInit {
  ...
  handleUpdate() {
    ...
    this.http.put("http://localhost:3005/api/staff/", updatedHousekeeper).subscribe(
      (resData) => {
        alert("updated");
      },
      (error) => {
  ...
  handleDelete(id: number) {
    ...
    if (confirmed) {
      this.http.delete(`http://localhost:3005/api/staff/remove/${id}`).subscribe(
        () => {
          this.housekeepers = this.housekeepers.filter(housekeeper => housekeeper.hid !== id);
          this.updateCurrentRecords();
        },
        (error) => {
